...
def agregar_Prov(request):
    if request.method == 'POST':
        ...
        if formProv.is_valid():
            formProv.save()
            return redirect("/agregar_prov/?valido")
    else:
...
def cuenta_Prov(request, filtro, id):
    if filtro == 0:
        ...
        cheques = chPropio.objects.filter(proveedor=proveedor_filtrado).order_by('pagado','fechaVto','monto')
        vto = date.today()
        ...
        cheques = chPropio.objects.filter(empresa=empresa_filtrada).order_by('pagado','fechaVto','monto')
        vto = date.today()
